...
function renderProducts(){
    productGrid.innerHTML=productRepo.all().filter(matchesFilter).map(p=>`<article class="card reveal ${p.stock===0?"sold-out":""}"><div class="image-wrap"><img src="${p.image}" onerror="productImageFallback(this,${p.id})" alt="${p.name}" loading="lazy">${p.line==="Novedades"?'<span class="new-tag">Nuevo</span>':""}</div><div class="card-row"><div><h3>${p.name}</h3><span class="muted">${p.category} · ${p.color}<br>Talla ${p.size}${p.sku?` · SKU ${p.sku}`:""}</span>${p.detail?`<p class="product-detail">${p.detail}</p>`:""}${p.officialUrl?`<a class="official-link" href="${p.officialUrl}" target="_blank" rel="noopener">Ver en Nike oficial</a>`:""}${stockText(p)}</div><b>${money(p.price)}</b></div><button ${p.stock===0?"disabled":""} onclick="add(${p.id})">${p.stock===0?"Producto agotado":"Agregar al carrito"}</button></article>`).join("")||"<p>No hay productos en esta sección.</p>";
    observeReveals();
}
function add(id){
